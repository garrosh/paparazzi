...
/**
 * @file firmwares/sam_test/autopilot.c
 *
 * Autopilot.
 *
 */
...
static void send_status(void) {
  ...
  DOWNLINK_SEND_SAM_TEST_STATUS(DefaultChannel, DefaultDevice,
...
static void send_fp(void) {
  ...
  DOWNLINK_SEND_SAM_TEST_FP(DefaultChannel, DefaultDevice,
...
static void send_sam_test_rc(void) {
...
static void send_sam_test_cmd(void) {
  DOWNLINK_SEND_SAM_TEST_CMD(DefaultChannel, DefaultDevice,
...
void autopilot_init(void) {
  ...
  register_periodic_telemetry(DefaultPeriodic, "SAM_TEST_STATUS", send_status);
  register_periodic_telemetry(DefaultPeriodic, "ENERGY", send_energy);
  register_periodic_telemetry(DefaultPeriodic, "SAM_TEST_FP", send_fp);
  register_periodic_telemetry(DefaultPeriodic, "SAM_TEST_CMD", send_sam_test_cmd);
  ...
  register_periodic_telemetry(DefaultPeriodic, "SAM_TEST_RADIO_CONTROL", send_sam_test_rc);
#endif
}
...
INFO("Using FAILSAFE_GROUND_DETECT: KILL")
  ...
  if (autopilot_mode == AP_MODE_KILL) {
    ...
    SetSam_TestCommands(stabilization_cmd, autopilot_in_flight, autopilot_motors_on);
  }
